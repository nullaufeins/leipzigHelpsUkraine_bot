...
const listener_on_text = async (bot, ctx, t, { debug, delete_calls, message_expiry }) => {
    // do nothing if bot:
    ...
    if (is_valid_communication_pre(text)) {
        ...
        if (commands.length > 0) {
            return universal_action(bot, ctx, commands[0], arguments, { debug, delete_calls });
        }
        // if invalid command, delete and return:
        return action_delete_and_ignore(bot, msg);
    } else if (text.startsWith(`/`)) {
        return listener_on_message(bot, ctx, t, { debug, message_expiry });
    }
...
const listener_on_message = async (bot, ctx, t, { debug, delete_calls, message_expiry }) => {
    // do nothing if bot:
    if (user_in_context_is_bot(ctx)) return action_ignore();
    // otherwise ...
    const msg = ctx.update.message;
